    ...
    <div className="m-8">
      <table className="min-w-full divide-y divide-gray-200 bg-white rounded-lg shadow-md">
        <thead className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
          <tr>
            <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider">Issue ID</th>
            ...
            <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider">Upvotes</th>
            <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider">Date Reported</th>
            <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-200">
          {issues.map((issue) => (
            <tr key={issue.id} className="hover:bg-gray-50 transition-all duration-200">
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{issue.id}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{issue.type}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{issue.category || 'N/A'}</td>
              <td className="px-6 py-4 text-sm text-gray-800">{issue.location || 'Not specified'}</td>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{issue.upvotes}</td>
...
const CorporationPanel = () => {
  const [issues] = useState(dummyData.issues); // Fetch from issues.json for now
