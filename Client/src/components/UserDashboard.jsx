  ...
  if (issues.length === 0) {
    return (
      <p className="text-gray-500 text-center">
        You haven’t reported any issues yet. Click "Raise an Issue" to get started!
      </p>
    );
  }
  return (
    <table className="min-w-full divide-y divide-gray-200">
      ...
      <tbody className="bg-white divide-y divide-gray-200">
        {issues.map((issue) => (
          <tr key={issue.id} className="hover:bg-gray-50 transition duration-200">
            ...
            <td className="px-6 py-4 whitespace-nowrap">{new Date(issue.date).toLocaleDateString()}</td>
            <td className="px-6 py-4 whitespace-nowrap">{issue.upvotes}</td>
            <td className="px-6 py-4 whitespace-nowrap">
              <Link to={`/issue/${issue.id}`} className="text-indigo-600 hover:text-indigo-900 transition duration-150">
                View Details
              </Link>
            </td>
          </tr>
...
const UserDashboard = () => {
  const [issues] = useState(dummyData.issues); // Dummy issues data
  ...
  const filteredIssues =
    activeTab === 'all' ? issues : issues.filter((issue) => issue.status.toLowerCase() === activeTab);
  ...
  return (
    <div className=" m-8 px-4 py-8 animate-fade-in">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mt-6">
        Welcome back, Anmol. You’ve reported {issues.length} issues so far.
      </h1>
      ...
      <section className="mt-8 m-8">
        <h2 className="text-2xl font-semibold text-gray-900">My Reported Issues</h2>
        <div className="mt-4 flex space-x-4">
          <button
            onClick={() => setActiveTab('all')}
            className={`px-3 py-1 rounded-md text-sm font-medium transition duration-150 ${
              activeTab === 'all' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
            All ({issues.length})
          </button>
          <button
            ...
            className={`px-3 py-1 rounded-md text-sm font-medium transition duration-150 ${
              activeTab === 'open' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1 rounded-md text-sm font-medium transition duration-150 ${
              activeTab === 'in-progress' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
            In Progress ({issues.filter((i) => i.status.toLowerCase() === 'in-progress').length})
          </button>
          <button
            onClick={() => setActiveTab('resolved')}
            className={`px-3 py-1 rounded-md text-sm font-medium transition duration-150 ${
              activeTab === 'resolved' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
      ...
      <section className="mt-8">
        <h2 className="text-2xl font-semibold text-gray-900">Types of Issues You Can Report</h2>
      ...
      <footer className="mt-8 text-center text-gray-500">
        <p>Your reports are anonymous and help improve your community. Thank you for contributing!</p>
      </footer>
    </div>
  );
};
